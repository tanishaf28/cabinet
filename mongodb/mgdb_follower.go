...
// the synthetic pool's split -- mirrors woc/objectmap.go's classifyRealKey
// and epaxos/mongodb's ClassifyRealKey exactly. Hashing the key (rather than
// rolling a fresh random number per access) keeps the type static per key:
// every client and replica computes the same type for the same key without
// coordination, and the real key is never substituted, so YCSB's actual
// Zipfian/latest access distribution and scan semantics are untouched.
