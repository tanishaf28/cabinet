...
func (m *PerfMeter) RecordStarter(pClock prioClock) {
	m.RecordStarterAt(pClock, time.Now())
}

func (m *PerfMeter) RecordStarterAt(pClock prioClock, startTime time.Time) {
	m.Lock()
	defer m.Unlock()
	if startTime.IsZero() {
		startTime = time.Now()
...
func (m *PerfMeter) SaveToFile() error {
	totalSlow := atomic.LoadInt64(&m.SlowCommits)
	totalConflict := atomic.LoadInt64(&m.ConflictCommits)
	...
	// positional indexing let the window invert, producing negative
	// totalWallClockSeconds and thus negative throughput.
	...
	for _, key := range keys {
		...
		end := value.StartTime.Add(time.Duration(value.TimeElapsed) * time.Millisecond)
		if earliestStart.IsZero() || value.StartTime.Before(earliestStart) {
			earliestStart = value.StartTime
		}
	...
	if counter == 0 {
		...
		err = writer.Write([]string{
			...
			"",
			"",
			strconv.FormatInt(totalSlow, 10) + " ops",
			strconv.FormatInt(totalConflict, 10) + " ops",
		})
		if err != nil {
			return err
		}
	...
	// actually finished by save time (see earliestStart/latestEnd above).
	totalWallClockSeconds := latestEnd.Sub(earliestStart).Seconds()
	...
	err = writer.Write([]string{"TOTAL_SLOW_COMMITS", strconv.FormatInt(totalSlow, 10), "ops"})
	if err != nil {
		return err
	}
	err = writer.Write([]string{"TOTAL_CONFLICT_COMMITS", strconv.FormatInt(totalConflict, 10), "ops"})
	if err != nil {
		return err
	}
