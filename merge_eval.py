...
def _default_output(eval_dir: str, role: str) -> str:
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return os.path.join(eval_dir, "merged", f"merged_cabinet_{role}s_{timestamp}.csv")


def _resolve_output(eval_dir: str, output: Optional[str], role: str) -> str:
    if not output:
        return _default_output(eval_dir, role)

    if os.path.isdir(output) or output.endswith(os.sep) or output.endswith("/"):
        return os.path.join(output, os.path.basename(_default_output(eval_dir, role)))
...
def _extract_dir_id(path: str, role: str) -> Optional[int]:
    name = os.path.basename(os.path.normpath(path))
    match = re.fullmatch(rf"{role}(\d+)", name)
    if not match:
        return None
    return int(match.group(1))


def _filter_role_dirs(role_dirs: list[str], allowed_ids: Optional[set[int]], role: str) -> list[str]:
    if allowed_ids is None:
        return role_dirs

    filtered: list[str] = []
    for d in role_dirs:
        node_id = _extract_dir_id(d, role)
        if node_id is not None and node_id in allowed_ids:
            filtered.append(d)
    return filtered
...
def merge_client_csvs(
    ...
    """Merge latest per-node Cabinet CSV files into one summary CSV.

    Cabinet client/server CSV format (from eval/meters.go):
    ...
    client_dirs_all = [
        d
        for d in glob.glob(os.path.join(eval_dir, f"{role}*/"))
        if os.path.isdir(d)
    ]
    client_dirs = _filter_role_dirs(client_dirs_all, allowed_client_ids, role)

    if not client_dirs:
        if allowed_client_ids is None:
            raise RuntimeError(f"No {role} directories found in {eval_dir}")
        raise RuntimeError(
            f"No matching {role} directories found in {eval_dir} for ids: {sorted(allowed_client_ids)}"
        )

    if allowed_client_ids is None:
        print(f"Found {len(client_dirs)} {role} directories")
    else:
        print(f"Found {len(client_dirs)} {role} directories matching ids {sorted(allowed_client_ids)}")
    ...
    if not all_latencies:
        raise RuntimeError(f"No latency data found in {role} CSV files")

    output_path = _resolve_output(eval_dir, output, role)
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(["NUM_CLIENT_DIRS", len(client_dirs), f"{role}* folders under eval"])
        writer.writerow(["NUM_CLIENTS_MERGED", merged_clients, f"{role}s with CSV data"])
        writer.writerow(["TOTAL_THROUGHPUT", f"{total_tpt:.1f} Tx/sec", f"sum of all {role}s"])
        writer.writerow(["AVG_LATENCY", f"{avg_lat:.3f} ms", f"across all batches all {role}s"])
    ...
    print(f"\nMerged {role} CSV written to: {output_path}")
    print(f"  {role.capitalize()}s merged: {merged_clients}/{len(client_dirs)}")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional comma/range filter for node IDs to merge. "
            "Examples: '5,6' or '5-10'. Targets client<ID> (or server<ID> with --servers)."
        ),
    )
    parser.add_argument(
        "--servers",
        action="store_true",
        help="Merge server*/ directories instead of client*/ directories.",
    )
    return parser
...
def main() -> int:
    ...
    try:
        ...
        merge_client_csvs(
            ...
            role="server" if args.servers else "client",
        )
        return 0
