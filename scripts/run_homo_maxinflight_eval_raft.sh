#!/bin/bash
# ================================================================
# MAX-INFLIGHT EVALUATION RUNNER (HOMOGENEOUS RAFT) — fixed 5/5
#
# Fixed 5-server/5-client cluster (config/cluster_homo.conf). Sweeps
# server-side MAX_INFLIGHT (Cabinet/Raft's -max-inflight is a server
# flag, not a client one -- see start_cluster_homo.sh's start_server).
# Homogeneous counterpart of run_hetero_maxinflight_eval_raft.sh.
#
# THRESHOLD (-t, quorum fault-tolerance) scales with cluster size as
# floor((n-1)/2) — 2 for n=5. See run_homo_maxinflight_eval_cab.sh for
# the Cabinet counterpart (THRESHOLD fixed at 1).
#
...
RESULT_ROOT="${REPO_ROOT}/results/homo_maxinflight_eval_raft"
...
Results archived under: results/homo_maxinflight_eval_raft/<timestamp>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║        MAX-INFLIGHT EVALUATION RUNNER (HOMOGENEOUS RAFT)        ║"
...
echo ""
echo "=================================================="
echo " Max-inflight sweep complete (homogeneous Raft)"
echo "=================================================="
