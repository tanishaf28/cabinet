#!/bin/bash
# ================================================================
# EVAL 6 (Raft): Batch Size Sweep x Cluster Size, 10-client pool
#
# Same sweep as run_hetero_plainmsg_raft.sh's eval2 (BATCHSIZE over
...
#
# Added alongside, not replacing, run_hetero_plainmsg_raft.sh's eval2 --
...
# unlike run_hetero_batchsize_sweep_10c_cab.sh, which fixes it at 1.
# ================================================================
...
RESULT_ROOT="${REPO_ROOT}/results/hetero_batchsize_sweep_10c_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_batchsize_sweep_10c_raft.sh

Sweeps BATCHSIZE over 1,10,50,100,500,1000,2000 (matches WOC's
eval_6_batchsize_size_sweep.sh / this repo's run_hetero_plainmsg_raft.sh
...
Results: results/hetero_batchsize_sweep_10c_raft/<timestamp>/n<N>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  BATCH SIZE SWEEP x CLUSTER SIZE, 10-CLIENT POOL (Raft)        ║"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Batch size sweep (10-client pool, Raft) complete              ║"
echo "╚════════════════════════════════════════════════════════════════╝"
