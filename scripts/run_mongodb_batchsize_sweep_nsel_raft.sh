#!/bin/bash
# ================================================================
# EVAL: MongoDB Batch Size Sweep x Cluster Size (Raft)
#
# Sweeps BATCHSIZE over 1,10,50,100,500,1000,2000 (same points/format as
# run_hetero_batchsize_sweep_10c_raft.sh's plain-msg sweep) with
...
# threshold, same convention as run_hetero_batchsize_sweep_10c_raft.sh --
# unlike run_mongodb_batchsize_sweep_nsel_cab.sh, which fixes it at 1.
#
...
USER="ubuntu"
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_batchsize_sweep_nsel_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_mongodb_batchsize_sweep_nsel_raft.sh
...
Results archived under: results/mongodb_batchsize_sweep_nsel_raft/<timestamp>/n<size>/<label>/
EOF
    exit 0
fi
...
cleanup_all_nodes() {
    ...
    done | sort -u > /tmp/mongobatch_raft_ip_pool.$$
    mapfile -t all_ips < /tmp/mongobatch_raft_ip_pool.$$
    rm -f /tmp/mongobatch_raft_ip_pool.$$
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║    RAFT MONGODB BATCH SIZE SWEEP x CLUSTER SIZE (n=3,5,7,11)     ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
echo ""
echo "=================================================="
echo " Raft MongoDB batch size sweep x cluster size complete"
echo "=================================================="
