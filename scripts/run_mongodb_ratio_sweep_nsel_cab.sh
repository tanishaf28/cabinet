#!/bin/bash
# ================================================================
# EVAL: MongoDB Indep-to-Dependent Ratio Sweep x Cluster Size (Cabinet)
#
# Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (same points/format as
# run_hetero_ratio_sweep_10c_cab.sh's plain-msg sweep) with BATCHSIZE=1,
...
# run_hetero_ratio_sweep_10c_cab.sh). See the sibling
# run_mongodb_ratio_sweep_nsel_raft.sh for the Raft-mode counterpart,
...
USER="ubuntu"
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_ratio_sweep_nsel_cab"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_mongodb_ratio_sweep_nsel_cab.sh
...
Results archived under: results/mongodb_ratio_sweep_nsel_cab/<timestamp>/n<size>/<label>/
EOF
    exit 0
fi
...
cleanup_all_nodes() {
    ...
    done | sort -u > /tmp/mongoratio_cab_ip_pool.$$
    mapfile -t all_ips < /tmp/mongoratio_cab_ip_pool.$$
    rm -f /tmp/mongoratio_cab_ip_pool.$$
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   CABINET MONGODB RATIO SWEEP x CLUSTER SIZE (n=3,5,7,11)       ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
echo ""
echo "=================================================="
echo " Cabinet MongoDB ratio sweep x cluster size complete"
echo "=================================================="
