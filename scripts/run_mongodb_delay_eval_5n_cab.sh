#!/bin/bash
# ================================================================
# MongoDB Network Delay Eval (Cabinet), n=5 heterogeneous, with timeseries
#
...
# interface, which also carries replies back to clients) against the
# Cabinet MongoDB cluster (start_mongodb_hetero_5n.sh, -et=1, fixed n=5,
# ENABLE_PRIORITY=true, THRESHOLD=1).
#
...
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_delay_eval_5n_cab"
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║ CABINET MONGODB DELAY EVAL, n=5 (server-side only, timeseries) ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for delay in "${DELAYS[@]}"; do
    ...
    ENABLE_TIMESERIES=true TPS_TIMELINE_INTERVAL_MS="${TPS_TIMELINE_INTERVAL_MS:-500}" \
        ...
        NUM_OBJECTS=1000 READ_RATIO=0 ENABLE_PRIORITY=true CONFIG_PATH="$CONFIG_PATH" \
...
echo ""
echo "=================================================="
echo " Cabinet MongoDB delay eval complete"
echo "=================================================="
