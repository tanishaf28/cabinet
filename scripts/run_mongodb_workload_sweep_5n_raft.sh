#!/bin/bash
# ================================================================
# Raft MongoDB Workload Sweep, HETEROGENEOUS 5-NODE (2s+3w) cluster.
#
# Sweeps YCSB WORKLOAD over a,b,c,d,e,f with INDEP_RATIO fixed at 90/10
# and BATCHSIZE=1, against the MongoDB-backed cluster
# (start_mongodb_hetero_5n.sh, -et=1, ENABLE_PRIORITY=false => Raft,
# standalone mongod per node). Mirrors run_mongodb_ratio_sweep_5n_raft.sh
...
# end -- same pattern as run_mongodb_delay_eval_5n_raft.sh). Also switched
...
USER="ubuntu"
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_workload_sweep_5n_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_mongodb_workload_sweep_5n_raft.sh

Sweeps YCSB WORKLOAD over a,b,c,d,e,f with INDEP_RATIO fixed (default 90)
and BATCHSIZE=1, against the heterogeneous 5-node Raft MongoDB cluster
(start_mongodb_hetero_5n.sh, -et=1, ENABLE_PRIORITY=false). Runs with
...
Results archived under: results/mongodb_workload_sweep_5n_raft/<timestamp>/<label>/
EOF
    exit 0
fi
...
run_case() {
    ...
    ENABLE_TIMESERIES=true TPS_TIMELINE_INTERVAL_MS="${TPS_TIMELINE_INTERVAL_MS:-500}" \
        INDEP_RATIO="$INDEP_RATIO_FIXED" BATCHSIZE=1 NUM_OBJECTS=1000 READ_RATIO=0 ENABLE_PRIORITY=false \
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   RAFT MONGODB WORKLOAD SWEEP, HETEROGENEOUS 5-NODE         ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
echo ""
echo "=================================================="
echo " Raft MongoDB workload sweep complete"
echo "=================================================="
