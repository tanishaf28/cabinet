#!/bin/bash
# ================================================================
# EVAL 5b (Cabinet): Indep-to-Dependent Ratio Sweep x Cluster Size,
# 10-client pool, READ_RATIO fixed at 100 (all reads)
#
# Read-heavy counterpart of run_hetero_ratio_sweep_10c_cab.sh: identical
...
# for every case instead of the default 0 (all writes). THRESHOLD stays
# fixed at 1 across all cluster sizes (Cabinet's quorum tolerance isn't
# tied to n the way Raft's is) -- see run_hetero_ratio_sweep_10c_read100_raft.sh
...
RESULT_ROOT="${REPO_ROOT}/results/hetero_ratio_sweep_10c_read100_cab"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_ratio_sweep_10c_read100_cab.sh
...
Results: results/hetero_ratio_sweep_10c_read100_cab/<timestamp>/n<N>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║ RATIO SWEEP x CLUSTER SIZE, 10-CLIENT POOL (Cabinet, read=100%)║"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Ratio sweep (10-client pool, Cabinet, read=100%) complete      ║"
echo "╚════════════════════════════════════════════════════════════════╝"
