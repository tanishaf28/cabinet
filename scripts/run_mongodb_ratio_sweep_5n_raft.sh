#!/bin/bash
# ================================================================
# Raft MongoDB Ratio Sweep, HETEROGENEOUS 5-NODE (2s+3w) cluster.
#
# Same as run_mongodb_ratio_sweep_5n_cab.sh but with ENABLE_PRIORITY=false
# (-ep=false => Raft instead of Cabinet on the same binary).
# ================================================================
...
USER="ubuntu"
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_ratio_sweep_5n_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_mongodb_ratio_sweep_5n_raft.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 with BATCHSIZE=1 fixed,
against the heterogeneous 5-node Raft MongoDB cluster
(start_mongodb_hetero_5n.sh, -et=1, ENABLE_PRIORITY=false).

Environment overrides:
  RUNTIME_SECONDS=30   wall-clock seconds per run
  WORKLOAD=a           YCSB workload letter (a-f)

Results archived under: results/mongodb_ratio_sweep_5n_raft/<timestamp>/<label>/
EOF
    exit 0
fi
...
run_case() {
    ...
    CLUSTER_ACTIVE=true
    INDEP_RATIO="$indep" BATCHSIZE=1 NUM_OBJECTS=1000 READ_RATIO=0 ENABLE_PRIORITY=false \
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   RAFT MONGODB RATIO SWEEP, HETEROGENEOUS 5-NODE               ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for indep in "${TEST_CASES[@]}"; do
    run_case "n5_raft_mongo_ratio_${indep}" "$indep"
done
...
echo ""
echo "=================================================="
echo " Raft MongoDB ratio sweep complete"
echo "=================================================="
