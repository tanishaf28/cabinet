#!/bin/bash
# ================================================================
# CLIENT SCALING EVALUATION RUNNER (HOMOGENEOUS RAFT)
#
...
# run_hetero_client_scaling_eval_raft.sh.
#
# THRESHOLD (-t, quorum fault-tolerance) scales with cluster size as
# floor((n-1)/2) — 2 for n=5. See run_homo_client_scaling_eval_cab.sh
...
RESULT_ROOT="${REPO_ROOT}/results/homo_client_scaling_eval_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_homo_client_scaling_eval_raft.sh

Sweeps total client count over 1,2,3,4,5,10,15,20,25,30,35,40,45,50 against a
fixed 5-server homogeneous Raft cluster (config/cluster_homo.conf), cycling
through the client-VM pool once the count exceeds its size. INDEP_RATIO=90,
BATCHSIZE=1, MSG_SIZE=512 fixed, THRESHOLD=floor((n-1)/2)=2 fixed.

Environment overrides:
  RUNTIME_SECONDS=30   wall-clock seconds per run

Results archived under: results/homo_client_scaling_eval_raft/<timestamp>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║          CLIENT SCALING EVALUATION RUNNER (HOMOGENEOUS RAFT)    ║"
