#!/bin/bash
# ================================================================
# HOMOGENEOUS PLAIN-MSG EVALUATION RUNNER (CABINET)
...
#
# Homogeneous counterpart of run_hetero_plainmsg_cab.sh. Same
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_homo_plainmsg_cab.sh [selector]
...
from config/cluster_homo.conf). THRESHOLD stays fixed at 1.
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║      HOMOGENEOUS PLAIN-MSG EVALUATION RUNNER (Cabinet)        ║"
