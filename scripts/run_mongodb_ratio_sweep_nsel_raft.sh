#!/bin/bash
# ================================================================
# EVAL: MongoDB Indep-to-Dependent Ratio Sweep x Cluster Size (Raft)
#
# Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (same points/format as
# run_hetero_ratio_sweep_10c_raft.sh's plain-msg sweep) with BATCHSIZE=1,
# MSG_SIZE=512 fixed, for each cluster size in CLUSTER_SIZES (default
# 3,5,7,11), against the MongoDB-backed cluster
# (start_mongodb_hetero_nsel.sh/stop_mongodb_hetero_nsel.sh, -et=1).
#
# THRESHOLD scales with cluster size as floor((n-1)/2) -- Raft's majority
# threshold, same convention as run_hetero_ratio_sweep_10c_raft.sh --
# unlike run_mongodb_ratio_sweep_nsel_cab.sh, which fixes it at 1.
#
...
USER="ubuntu"
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_ratio_sweep_nsel_raft"
...
WORKLOAD="${WORKLOAD:-a}"
TEST_CASES=(100 90 80 60 40 20 10 0)
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_mongodb_ratio_sweep_nsel_raft.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 with BATCHSIZE=1,
MSG_SIZE=512 fixed, across cluster sizes n=3,5,7,11, against the Raft
MongoDB cluster (start_mongodb_hetero_nsel.sh, -et=1, ENABLE_PRIORITY=false,
THRESHOLD=floor((n-1)/2) per size).
...
Results archived under: results/mongodb_ratio_sweep_nsel_raft/<timestamp>/n<size>/<label>/
EOF
    exit 0
fi
...
cleanup_all_nodes() {
    ...
    done | sort -u > /tmp/mongoratio_raft_ip_pool.$$
    mapfile -t all_ips < /tmp/mongoratio_raft_ip_pool.$$
    rm -f /tmp/mongoratio_raft_ip_pool.$$
...
run_case() {
    ...
    echo "=================================================="
    echo "Running: $label"
    echo "  n=${CURRENT_N}  t=${THRESHOLD}  indep=${indep}  workload=${WORKLOAD}  runtime=${RUNTIME_SECONDS}s"
    echo "=================================================="

    CLUSTER_ACTIVE=true
    NUM_SERVERS="$CURRENT_N" THRESHOLD="$THRESHOLD" INDEP_RATIO="$indep" BATCHSIZE=1 MSG_SIZE=512 \
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   RAFT MONGODB RATIO SWEEP x CLUSTER SIZE (n=3,5,7,11)          ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for n in "${ALL_CLUSTER_SIZES[@]}"; do
    ...
    echo ""
    echo "=================================================="
    echo " Cluster size n=${CURRENT_N} (t=${THRESHOLD}, Raft majority)"
    echo "=================================================="
    for indep in "${TEST_CASES[@]}"; do
        run_case "n${n}_mongo_ratio_${indep}" "$indep"
    done
done
...
echo ""
echo "=================================================="
echo " Raft MongoDB ratio sweep x cluster size complete"
echo "=================================================="
