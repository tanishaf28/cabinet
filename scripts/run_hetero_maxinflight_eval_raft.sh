#!/bin/bash
# ================================================================
# MAX-INFLIGHT EVALUATION RUNNER (Raft) — 10-client pool
#
# Sweeps server-side MAX_INFLIGHT (Cabinet/Raft's -max-inflight is a
# server flag, not a client one -- see start_cluster_hetero.sh's
# start_server) across cluster sizes n=3,5,7,11 against the dedicated
# 10-VM client pool (config/cluster_hetero_{n}n_10c.conf), mirroring
# WOC's run_hetero_maxinflight_eval.sh / this repo's
# run_hetero_ratio_sweep_10c_raft.sh.
#
# THRESHOLD (-t, quorum fault-tolerance) scales with cluster size as
# floor((n-1)/2) -- Raft's majority threshold -- unlike
# run_hetero_maxinflight_eval_cab.sh, which fixes it at 1.
#
# For a fixed 5-server/5-client run: CLUSTER_SIZES=5 CLIENT_COUNT=5 bash
# run_hetero_maxinflight_eval_raft.sh
# ================================================================
...
RESULT_ROOT="${REPO_ROOT}/results/hetero_maxinflight_eval_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_maxinflight_eval_raft.sh
...
(config/cluster_hetero_{n}n_10c.conf). THRESHOLD scales as
floor((n-1)/2) (Raft majority semantics).
...
Results: results/hetero_maxinflight_eval_raft/<timestamp>/n<N>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   MAX-INFLIGHT SWEEP, 10-CLIENT POOL (Raft)                    ║"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Max-inflight sweep (10-client pool, Raft) complete             ║"
echo "╚════════════════════════════════════════════════════════════════╝"
