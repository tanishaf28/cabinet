#!/bin/bash
# ================================================================
# CLIENT SCALING EVALUATION RUNNER (HETEROGENEOUS CABINET)
#
...
# (run_hetero_plainmsg_cab.sh, run_hetero_crash_cab.sh).
#
...
# run_hetero_client_scaling_eval_raft.sh for the Raft counterpart.
#
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_client_scaling_eval_cab.sh

Sweeps total client count over 1,2,3,4,5,10,15,20,25,30,35,40,45,50 against a
fixed 5-server heterogeneous Cabinet cluster, packing up to 2 client
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║      CLIENT SCALING EVALUATION RUNNER (HETEROGENEOUS CABINET)   ║"
