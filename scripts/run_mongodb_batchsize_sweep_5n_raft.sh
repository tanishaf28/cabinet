#!/bin/bash
# ================================================================
# Raft MongoDB Batch Size Sweep, HETEROGENEOUS 5-NODE (2s+3w) cluster.
#
# Same as run_mongodb_batchsize_sweep_5n_cab.sh but with
# ENABLE_PRIORITY=false (-ep=false => Raft instead of Cabinet).
# ================================================================
...
USER="ubuntu"
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_batchsize_sweep_5n_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_mongodb_batchsize_sweep_5n_raft.sh

Sweeps BATCHSIZE over 1,10,50,100,500,1000,2000 with INDEP_RATIO="${INDEP_RATIO:-90.0}"
fixed, against the heterogeneous 5-node Raft MongoDB cluster
...
Results archived under: results/mongodb_batchsize_sweep_5n_raft/<timestamp>/<label>/
EOF
    exit 0
fi
...
run_case() {
    ...
    CLUSTER_ACTIVE=true
    INDEP_RATIO="${INDEP_RATIO:-90.0}" BATCHSIZE="$batch_size" NUM_OBJECTS=1000 READ_RATIO=0 ENABLE_PRIORITY=false \
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   RAFT MONGODB BATCH SIZE SWEEP, HETEROGENEOUS 5-NODE          ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for batch_size in "${TEST_CASES[@]}"; do
    run_case "n5_raft_mongo_batch_${batch_size}" "$batch_size"
done
...
echo ""
echo "=================================================="
echo " Raft MongoDB batch size sweep complete"
echo "=================================================="
