#!/bin/bash
# ================================================================
# EVAL 5 (Raft): Indep-to-Dependent Ratio Sweep x Cluster Size,
# 10-client pool
#
# Same sweep as run_hetero_plainmsg_raft.sh's eval1 (INDEP_RATIO over
...
# epaxos's run_hetero_ratio_sweep_10c.sh.
#
# Added alongside, not replacing, run_hetero_plainmsg_raft.sh's eval1 --
# that script and its 2-client configs are untouched. THRESHOLD scales
# with cluster size as floor((n-1)/2) -- Raft's majority threshold --
# unlike run_hetero_ratio_sweep_10c_cab.sh, which fixes it at 1.
# ================================================================
...
RESULT_ROOT="${REPO_ROOT}/results/hetero_ratio_sweep_10c_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_ratio_sweep_10c_raft.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (matches WOC's
eval_5_i2d_size_sweep.sh / this repo's run_hetero_plainmsg_raft.sh eval1)
...
THRESHOLD scales as floor((n-1)/2) (Raft majority semantics).
...
Results: results/hetero_ratio_sweep_10c_raft/<timestamp>/n<N>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   RATIO SWEEP x CLUSTER SIZE, 10-CLIENT POOL (Raft)            ║"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Ratio sweep (10-client pool, Raft) complete                   ║"
echo "╚════════════════════════════════════════════════════════════════╝"
