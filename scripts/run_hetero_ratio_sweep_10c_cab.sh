#!/bin/bash
# ================================================================
# EVAL 5 (Cabinet): Indep-to-Dependent Ratio Sweep x Cluster Size,
# 10-client pool
#
# Same sweep as run_hetero_plainmsg_cab.sh's eval1 (INDEP_RATIO over
...
#
# Added alongside, not replacing, run_hetero_plainmsg_cab.sh's eval1 --
...
# tied to n the way Raft's is) -- see run_hetero_ratio_sweep_10c_raft.sh
...
RESULT_ROOT="${REPO_ROOT}/results/hetero_ratio_sweep_10c_cab"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_ratio_sweep_10c_cab.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (matches WOC's
eval_5_i2d_size_sweep.sh / this repo's run_hetero_plainmsg_cab.sh eval1)
...
THRESHOLD fixed at 1 (Cabinet quorum semantics).
...
Results: results/hetero_ratio_sweep_10c_cab/<timestamp>/n<N>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   RATIO SWEEP x CLUSTER SIZE, 10-CLIENT POOL (Cabinet)         ║"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Ratio sweep (10-client pool, Cabinet) complete                ║"
echo "╚════════════════════════════════════════════════════════════════╝"
