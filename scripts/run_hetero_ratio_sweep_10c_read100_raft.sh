#!/bin/bash
# ================================================================
# EVAL 5b (Raft): Indep-to-Dependent Ratio Sweep x Cluster Size,
# 10-client pool, READ_RATIO fixed at 100 (all reads)
#
# Read-heavy counterpart of run_hetero_ratio_sweep_10c_raft.sh: identical
# sweep (INDEP_RATIO over 100,90,80,60,40,20,10,0, batch=1, msgsize=512,
# n=3,5,7,11, dedicated 10-VM client pool) but with READ_RATIO=100 fixed
# for every case instead of the default 0 (all writes), mirroring the
# same READ_RATIO=100 fixed point used by the woc/cabinet/epaxos
# read-ratio sweeps' upper bound. THRESHOLD scales with cluster size as
# floor((n-1)/2) -- Raft's majority threshold -- unlike
# run_hetero_ratio_sweep_10c_read100_cab.sh, which fixes it at 1.
# ================================================================
...
RESULT_ROOT="${REPO_ROOT}/results/hetero_ratio_sweep_10c_read100_raft"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_ratio_sweep_10c_read100_raft.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 with READ_RATIO fixed at
100 (all reads), BATCHSIZE=1, MSG_SIZE=512 fixed, across cluster sizes
n=3,5,7,11, against a dedicated 10-VM client pool
(config/cluster_hetero_{n}n_10c.conf). THRESHOLD scales as floor((n-1)/2)
(Raft majority semantics).
...
Results: results/hetero_ratio_sweep_10c_read100_raft/<timestamp>/n<N>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   RATIO SWEEP x CLUSTER SIZE, 10-CLIENT POOL (Raft, read=100%) ║"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Ratio sweep (10-client pool, Raft, read=100%) complete         ║"
echo "╚════════════════════════════════════════════════════════════════╝"
