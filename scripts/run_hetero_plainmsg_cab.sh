#!/bin/bash
# ================================================================
# HETEROGENEOUS PLAIN-MSG EVALUATION RUNNER (CABINET)
...
#
# THRESHOLD (-t) stays fixed at 1 across all cluster sizes — Cabinet's
# quorum tolerance isn't tied to n the way Raft's majority threshold is.
#
# Max-inflight, crash/fault-injection, and network-delay sweeps live
# in their own dedicated scripts now (run_hetero_maxinflight_cab.sh,
# run_hetero_crash_cab.sh, run_hetero_netem_cab.sh) — mirroring woc's
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_plainmsg_cab.sh [selector]
...
config/cluster_hetero_<n>n_*.conf). THRESHOLD stays fixed at 1.
...
Other sweeps now live in dedicated scripts:
  run_hetero_maxinflight_cab.sh   client max-inflight sweep
  run_hetero_crash_cab.sh         fault injection (leader/follower/f-of-n)
  run_hetero_netem_cab.sh         network delay (D1 uniform + D4 burst)
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║      HETEROGENEOUS PLAIN-MSG EVALUATION RUNNER (Cabinet)      ║"
